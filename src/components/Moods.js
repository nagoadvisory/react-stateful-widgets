...
export default function Moods() {
  const [Moood, setMoood] = useState(initialMood, happyMood, sadMood)
  /* STEP 1 */
  ...
  const reset = () => {
    /* STEP 6 */
    {setMoood(initialMood)}
  };

  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: Moood == happyMood ? 'crimson': 'royalblue' /* STEP 2 royal blue */
  };

  return (
    <div className='widget-moods container'>
      <h2>Moods</h2>
      <div id='mood' style={style}>{Moood}</div> {/* STEP 3 */}
      <div>
